...
ノートが100枚を超えた頃、ユーザーが望めば全文検索インデックス、Vaultのgit管理、孤児・description欠落チェックを提案する。最初からは導入しない。全文検索インデックスと孤児・description欠落チェックは追加パッケージ [addons/kura-index](addons/kura-index/README.md) として用意している。
